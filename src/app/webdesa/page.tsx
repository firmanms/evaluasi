...
import {
  ...
  Activity, Users, Layers, Info, CheckCircle2, XCircle, ArrowUpRight, Lock
} from "lucide-react";
import {
...
// Konfigurasi: Atur ke false jika ingin halaman ini langsung terbuka tanpa PIN
const REQUIRE_PIN = true;

export default function WebDesaPublicDashboard() {
  const [isAuthenticated, setIsAuthenticated] = useState(!REQUIRE_PIN);
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "var(--background)", padding: 20 }}>
        <div className="card animate-fade-in" style={{ padding: "40px", maxWidth: 400, width: "100%", textAlign: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 64, height: 64, borderRadius: 32, background: "rgba(59,130,246,0.1)", color: "#3b82f6", marginBottom: 24 }}>
            <Lock size={32} />
          </div>
          <h2 style={{ fontSize: 24, fontWeight: 700, margin: "0 0 8px 0" }}>Akses Dashboard</h2>
          <p style={{ color: "var(--muted-foreground)", marginBottom: 32, fontSize: 14 }}>
            Silakan masukkan kode PIN akses untuk melihat Dashboard Web Desa.
          </p>
          <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <input
              type="password"
              placeholder="Masukkan kode (misal: 3204)"
              value={passcode}
              onChange={(e) => setPasscode(e.target.value)}
              style={{
                width: "100%", padding: "12px 16px", borderRadius: 8,
                border: authError ? "1px solid #ef4444" : "1px solid var(--border)",
                background: "var(--background)", color: "var(--foreground)", fontSize: 16, textAlign: "center"
              }}
              autoFocus
            />
            {authError && <div style={{ color: "#ef4444", fontSize: 13, marginTop: -8 }}>Kode akses tidak valid.</div>}
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "var(--background)", color: "var(--muted-foreground)" }}>
  ...
  if (error) {
    return (
      <div style={{ display: "flex", minHeight: "100vh", padding: 24, background: "var(--background)" }}>
        <div className="card" style={{ padding: 40, textAlign: "center", color: "#ef4444", margin: "auto" }}>
      ...
      <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <p className="page-subtitle">Statistik infrastruktur, versi OpenSID, dan status pengelolaan website desa Kabupaten Bandung</p>
          </div>
          <div>
            {REQUIRE_PIN && (
              <button onClick={() => setIsAuthenticated(false)} className="btn btn-secondary btn-sm">Keluar (Lock)</button>
            )}
          </div>
        </div>
